...
library(readr)
churn_data <- read_csv("data/churn_data.csv")
View(churn_data)
str(churn_data)
churn_data$Churn <- as.factor(churn_data$Churn)
rand_ind <- sample(seq_len(nrow(churn_data)), 500)
churns_dataset <- churn_data[rand_ind, ]
# Define a 70:30 train:test data split of the dataset.
train_index <- createDataPartition(churns_dataset$Churn,
                                   p = 0.7,
                                   list = FALSE)
churn_train <- churns_dataset[train_index, ]
churn_test <- churns_dataset[-train_index, ]

#### Train the model ----
churn_model_glm <- glm(Churn ~ tenure,MonthlyCharges,TotalCharges, data = churn_train,
